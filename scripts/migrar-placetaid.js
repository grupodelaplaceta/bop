#!/usr/bin/env node
/* ═══════════════════════════════════════════════════════════════════════
   BOP — Publicación del documento legal de PlacetaID Móvil
   PM-PRV-001 (Política de Privacidad) — el que se enlaza en Google Play.
   ...
   URL pública resultante:
     https://bop.laplaceta.org/documento?codigo=PM-PRV-001

   Ejecutar:  node scripts/migrar-placetaid.js
   ═══════════════════════════════════════════════════════════════════════ */
const fs = require('fs');
...
const DOCUMENTOS = [
  {
    ...
    notas_cambio: 'Publicación de la Política de Privacidad de PlacetaID Móvil v26.8.1, requerida para su publicación en Google Play (Google Play Console exige una URL pública de política de privacidad).',
    archivo: 'placetaid-movil-politica-de-privacidad.md'
  }
];
...
async function main() {
  ...
  for (const doc of DOCUMENTOS) {
    ...
    const verBody = {
      ...
      autor_dip: AUTOR.dip,
      autor_nombre: AUTOR.nombre,
      notas_cambio: 'Publicación inicial de la Política de Privacidad de PlacetaID Móvil v26.8.1.',
      fecha_propuesta: doc.fecha_aplicacion,
      fecha_aprobacion_junta: doc.fecha_aprobacion_junta,
      aprobada_en_junta: doc.aprobada_en_junta
    };
  ...
  console.log(`Migración PlacetaID Móvil completada: ${ok} publicados, ${errs} errores.`);
  console.log('URL pública: https://bop.laplaceta.org/documento?codigo=PM-PRV-001');
}
